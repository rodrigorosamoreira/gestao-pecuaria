...
const InventoryManager: React.FC<InventoryManagerProps> = ({ inventory, onAddStock, onUpdateStock }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (currentItem.id) {
      ...
      onAddStock({ ...currentItem, id: `${Date.now()}-${Math.random().toString(36).substring(2, 7)}` });
    }
    closeModal();
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredItems.map((item, idx) => {
          ...
          return (
            <div key={`${item.id}-${idx}`} className={`bg-white p-6 rounded-xl shadow-sm border-2 ${isLowStock ? 'border-red-100' : 'border-transparent'}`}>
