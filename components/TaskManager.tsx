      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredTasks.length > 0 ? (
          filteredTasks.map((task, idx) => (
            <div key={`${task.id}-${idx}`} className={`bg-white rounded-2xl p-6 shadow-sm border transition-all hover:shadow-md ${task.status === 'Concluída' ? 'opacity-60 border-gray-100' : 'border-gray-100'}`}>
